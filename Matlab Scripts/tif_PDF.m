...
show_tif_pdf(tif1, 1, xlims, ylims, 'No Iron', bits)
show_tif_pdf(tif2, 1, xlims, ylims, 'Iron', bits)
show_tif_pdf(tif3, 2, xlims, ylims, '2 thin sheets, few iron strands', bits)
show_tif_pdf(tif4, 1, xlims, ylims, '2 Sheets, one full iron, one no iron', bits)
show_tif_pdf(tif5, 2, xlims, ylims, '1 thin Sheet ', bits)

% TIF must be in the directory of this script to show
function show_tif_pdf(tif, fig_num, xlims, ylims, title_text, bits)
    figure(fig_num);
    tiff = imread(tif);
    ...
    if bits == 8
        res = 2^8;
        counts = histcounts(tiff,res);  % currently broken, things are not shifted correctly
    else
        res = 2^16;
